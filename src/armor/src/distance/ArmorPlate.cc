...
)
{
    ...
    for(size_t i = 0 ; i < matches.size() ; ++i){
        ...
        at::Tensor res = clf.forward({ts}).toTensor().clamp(0, 1).round();
        int v1 = res.index({0, 0}).item().toInt();
        int v2 = res.index({0, 1}).item().toInt();
...
void ArmorPlate::drawArmorPlates(cv::Mat &src, 
    ...
    for (size_t i = 0; i< tar_list.size(); ++i) {
        if(tar_list[i].armor_number != -1){   //有意义的数字
            int line_width = tar_list[i].Isbigarmor ? 3 : 1;
